...
export const RevenueSummary: React.FC<RevenueSummaryProps> = ({ propertyId = 'prop-001', debugTenant, showRaw }) => {
    ...
    const [intPart, fracPart = '00'] = data.total_revenue.split('.');
    const formattedInt = Number(intPart).toLocaleString();
    const displayTotal = `${formattedInt}.${fracPart.padEnd(2, '0').slice(0, 2)}`;
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-300">
            ...
            <div className="p-6">
                ...
                <div className="mt-4 h-6">
                    {showRaw && data.total_revenue.includes('.') && data.total_revenue.split('.')[1].length > 2 && (
                        <div className="flex items-center text-xs text-amber-600 bg-amber-50 px-2 py-1 rounded">
                            <svg className="h-4 w-4 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                            </svg>
                            Sub-cent precision rounded for display
                        </div>
                    )}
                </div>
            </div>
